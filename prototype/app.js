...
function startLoadingAnimation() {
  const texts = ['匹配上海消费数据…', '推算预算区间…', '生成专属建议…'];
...
function bindNavigation() {
  document.addEventListener('click', (e) => {
    ...
    const radioChip = e.target.closest('.radio-chip');
    if (radioChip) {
      const group = radioChip.closest('.radio-row');
      if (group) {
    ...
    if (chip && chip.closest('.action-btns')) {
      chip.closest('.action-btns').querySelectorAll('.chip').forEach((c) => c.classList.remove('active'));

...
export default {
  onLaunch() {
    initCloud() // wx.cloud.init({ env, traceUser: true })
    this.bootstrapUser()
  },
  methods: {
    async bootstrapUser() {
...
const res = await cloud.cloudPay({
  ...
  functionData: { order_id },
  ...paymentParams
})
return { payment: res.payment, order_id }
```
...
## 附录 C：initDb 云函数（仅 staging 执行一次）

```javascript
// 创建索引（示例）
db.collection('users').createIndex({ _openid: 1 }, { unique: true })
db.collection('weekly_entries').createIndex({ family_id: 1, week_start: 1 }, { unique: true })
db.collection('budget_plans').createIndex({ family_id: 1, is_active: 1 })
```

...
## 〇、当前实际进度（2026-09-06 更新）

> 正文各 Phase 的验收复选框**未逐项更新**，以本节为准。
> 逐项勾选维护成本高且很快过期，故只维护状态摘要与偏差说明。

| Phase | 内容 | 状态 | 说明 |
|-------|------|------|------|
| 0 | 工程基础 | ✅ 完成 | |
| 1 | 测算引擎 + 基准数据 | ✅ 完成 | 城市已扩至 30 城 tier3 体系（超出原 10 城计划） |
| 2 | 核心 API 层 | ✅ 完成 | 29 个 action |
| 3 | 免登录快测 UI | ✅ 完成 | |
| 4 | 完整测算向导 | ✅ 完成 | |
| 5 | 规划书 + 建议规则 | ✅ 完成 | 14 条规则 R01–R07 + R-N01–N07 |
| 6 | 登录 + 数据持久化 | ✅ 完成 | **含 1 项偏差，见下** |
| 7 | 预算追踪闭环 | ✅ 完成 | |
| 8 | 分享 | ✅ 完成 | **商业化部分（支付/订阅）已取消** |
| 9 | 内测与校准 | ⏸ 未开始 | 引擎系数仍为占位估算，待内测数据校准 |
| 10 | P1 增强 | ✅ 基本完成 | 伴侣协同、月末复盘（含历史月份回看）、订阅消息、分享 |

### 已确认的偏差

1. **商业化取消（2026-08）**：Phase 8 的支付 / 订阅 / 付费墙整体不做，
   M7 里程碑「付费解锁」作废。详见 PDD §9.1–9.3 的作废说明。
2. **敏感字段加密未做**：Phase 6 原验收标准含「敏感字段（income）DB 加密存储」，
   **未实现**，且经评估决定不做 —— 理由见 §2.5。
...
```
jiajitong/
├── uniapp/                   # UniApp + 微信云开发正式工程
│   ├── src/                  # UniApp 源码（pages/components/stores/services）
│   └── cloudfunctions/       # 云函数（api 网关 + common/engine + common/benchmark-data）
├── docs/
├── prototype/                # v1 HTML 原型（设计参考）
└── prototype-v2/             # v2 HTML 原型（开发对齐）
```

> 正式工程按 [技术方案 §〇 工程现状与迁移](技术方案-UniApp云开发.md) 新建；公共模块路径统一为 `cloudfunctions/common/`（引擎与基准数据），见技术方案 §3、§0.3。
...
### 2.5 关于「敏感字段加密存储」的决策（2026-09-06）

Phase 6 验收标准原含「敏感字段（income）DB 加密存储」。**经评估决定不实现**，
改为依赖既有控制 + 数据最小化。理由如下：

**现有防护（已足够且是正确的一层）**

1. 全部集合权限设为「自定义安全规则 全 false」——**客户端完全无法直连数据库**，
   所有读写经云函数管理员权限执行（见《云数据库初始化指南》§三）。
2. 云函数层逐个 action 做 `requireAuth` + 归属校验（`family_id` / `requireOwner`），
   `plans.getById` 等按 id 取数据的接口一律校验归属，越权按不存在处理。
3. 云开发自带存储层加密与传输加密。

**为什么不做应用层字段加密**

- 密钥只能放在云函数环境变量，而**能访问数据库控制台的人同样能读到环境变量**，
  防护增益有限，属"把锁和钥匙放在一起"。
- 应用层加密会让 `plan_input` 变成不可读字节串，一旦密钥丢失或轮换失误，
  **数据不可逆损坏**；而 `plan_input` 是引擎重算（`plans.recalc`）的必要输入。
- 真正有效的方案是外部 KMS + 信封加密，对当前体量属过度设计。

**替代措施（已实施）**

- 被新版取代的旧方案不再保留 `plan_input`（原始收入/支出输入），
  只保留测算结果用于回看 —— 减少敏感数据留存面。
  见 `db.savePlan()` 的 `stripPlanInputFromSuperseded`。
- 数据导出（`users.exportData`）与注销（`users.deleteMe`）均已实现，
  满足《个人信息保护法》的查阅复制权与删除权。

> 若日后合规要求提升（如需对外承诺字段级加密），再按 KMS 方案重做，
> 届时本节应一并更新。
...
| # | 任务 | 说明 |
|---|------|------|
| 1.1 | `cloudfunctions/common/benchmark-data` | 10 城 × 7 大类人均月消费（PDD 附录 B） |
...
- [ ] `cloudfunctions/common/engine` 测试覆盖率 ≥ 85%（核心计算函数）
...
| # | 任务 |
|---|------|
| 5.1 | `cloudfunctions/common/engine/rules` — R01–R07 + R-N01–N07 |
...
| W12 | 城市扩 30 城 + tier3 体系 | benchmark seed + tier3Multiplier | 新城测算可用 + tier3 适配回归测试 |
